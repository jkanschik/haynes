  ...
  def self.up
    [ Abbreviation, Addition, Alia, Blog, Code, Composer, ComposersWorkVersion, ComposersWork, Country,
      ...
      klazz.send(:columns).each do |column|
        if column.text?
          execute "alter table #{klazz.table_name} modify #{column.name} blob"
          execute "alter table #{klazz.table_name} modify #{column.name} #{column.sql_type} character set utf8"
          execute "alter table #{klazz.table_name} modify #{column.name} #{column.sql_type} character set latin1 collate latin1_german2_ci"
        end
      end
    end
  end
